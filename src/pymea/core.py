"""Shared state containers and electrode geometry."""
...
class ElectrodeConfig:
    """Electrode coordinates for the supported 120- and 60-channel layouts."""
    ...
    def electrode_toggle(self, raw_data):
        if raw_data.new_data_size[1] > 100:
            ...
            self.electrode_coords_x = np.array(
                [coord[0] for coord in self.mea_120_coordinates.values()]
            )
            self.electrode_coords_y = np.array(
                [coord[1] for coord in self.mea_120_coordinates.values()]
            )
        else:
            self.electrode_names = list(self.mea_60_coordinates.keys())
            self.electrode_coords_x = np.array(
                [coord[0] for coord in self.mea_60_coordinates.values()]
            )
            self.electrode_coords_y = np.array(
                [coord[1] for coord in self.mea_60_coordinates.values()]
            )


def create_analysis_state():
    """Instantiate the mutable containers used by the legacy analysis modules."""
